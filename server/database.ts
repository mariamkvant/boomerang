...
try { await client.query("ALTER TABLE help_wanted DROP CONSTRAINT IF EXISTS help_wanted_status_check"); } catch(e) {}
    ...
    try { await client.query('ALTER TABLE users ADD COLUMN IF NOT EXISTS streak_weeks INTEGER DEFAULT 0'); } catch(e) {}
    try { await client.query('ALTER TABLE users ADD COLUMN IF NOT EXISTS last_streak_week TEXT'); } catch(e) {}
    try { await client.query('ALTER TABLE reviews ADD COLUMN IF NOT EXISTS provider_reply TEXT'); } catch(e) {}
    try { await client.query('ALTER TABLE reviews ADD COLUMN IF NOT EXISTS provider_reply_at TIMESTAMPTZ'); } catch(e) {}
    try { await client.query('ALTER TABLE reviews ADD COLUMN IF NOT EXISTS image TEXT'); } catch(e) {}
    try { await client.query('ALTER TABLE services ADD COLUMN IF NOT EXISTS city TEXT'); } catch(e) {}
    try { await client.query('ALTER TABLE services ADD COLUMN IF NOT EXISTS country TEXT DEFAULT \'Luxembourg\''); } catch(e) {}
    ...
    // Backfill: ensure no NULLs in is_product — runs every startup, safe idempotent operation
    try { await client.query("UPDATE services SET is_product = false WHERE is_product IS NULL"); } catch(e) {}
    // Ensure is_product column has proper default
    try { await client.query("ALTER TABLE services ALTER COLUMN is_product SET DEFAULT false"); } catch(e) {}
