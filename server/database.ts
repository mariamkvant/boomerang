...
export async function initDatabase() {
  ...
  try {
    ...
    await client.query('CREATE TABLE IF NOT EXISTS reports (id SERIAL PRIMARY KEY, reporter_id INTEGER NOT NULL REFERENCES users(id), reported_id INTEGER NOT NULL REFERENCES users(id), reason TEXT NOT NULL, details TEXT DEFAULT try { await client.query('ALTER TABLE users ADD COLUMN IF NOT EXISTS trust_score INTEGER DEFAULT 0'); } catch(e) {}q$, status TEXT DEFAULT $, created_at TIMESTAMPTZ DEFAULT NOW())');
