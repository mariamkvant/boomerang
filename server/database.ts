...
export async function initDatabase() {
  ...
  try {
    ...
    try { await client.query('ALTER TABLE services ADD COLUMN IF NOT EXISTS group_id INTEGER REFERENCES groups(id)'); } catch(e) {}
            await client.query('CREATE TABLE IF NOT EXISTS groups (id SERIAL PRIMARY KEY, name TEXT UNIQUE NOT NULL, description TEXT DEFAULT await client.query('CREATE TABLE IF NOT EXISTS notificationsq$, is_public BOOLEAN DEFAULT true, invite_code TEXT UNIQUE, created_by INTEGER NOT NULL REFERENCES users(id), created_at TIMESTAMPTZ DEFAULT NOW())');
    await client.query('CREATE TABLE IF NOT EXISTS group_members (id SERIAL PRIMARY KEY, group_id INTEGER NOT NULL REFERENCES groups(id), user_id INTEGER NOT NULL REFERENCES users(id), role TEXT DEFAULT $, joined_at TIMESTAMPTZ DEFAULT NOW(), UNIQUE(group_id, user_id))');
