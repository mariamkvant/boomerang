...
// Get all categories with multipliers
...
router.get('/categories/:id/subcategories', (req: AuthRequest, res: Response) => {
  const subs = db.all('SELECT * FROM subcategories WHERE category_id = ? ORDER BY name', req.params.id);
  res.json(subs);
});

// Get all subcategories (for browse filtering)
router.get('/subcategories', (_req, res: Response) => {
  const subs = db.all('SELECT s.*, c.name as category_name, c.icon as category_icon FROM subcategories s JOIN categories c ON s.category_id = c.id ORDER BY c.name, s.name');
  res.json(subs);
});

// Calculate suggested points for a service
router.get('/calculate-points', (req: AuthRequest, res: Response) => {
  ...
  const cat = db.get('SELECT * FROM categories WHERE id = ?', category_id);
  ...
  res.json({ suggested, min, max, multiplier: cat.multiplier, base_rate: cat.base_rate });
});
...
router.get('/', (req: AuthRequest, res: Response) => {
  ...
  let query = `
    SELECT s.*, c.name as category_name, c.icon as category_icon, c.multiplier,
  ...
  if (subcategory) { query += ' AND s.subcategory_id = ?'; params.push(subcategory); }
  if (search) { query += ' AND (s.title LIKE ? OR s.description LIKE ?)'; params.push(`%${search}%`, `%${search}%`); }

  query += ' ORDER BY s.created_at DESC LIMIT ? OFFSET ?';
  params.push(limit, offset);

  const services = db.all(query, ...params);
  res.json(services);
});
...
router.get('/:id', (req: AuthRequest, res: Response) => {
  ...
  const reviews = db.all(`
    ...
    JOIN users u ON r.reviewer_id = u.id JOIN service_requests sr ON r.request_id = sr.id
    WHERE sr.service_id = ? ORDER BY r.created_at DESC
  `, req.params.id);
...
router.post('/', authMiddleware, (req: AuthRequest, res: Response) => {
  ...
  // Auto-calculate if no points_cost provided
  let finalPoints = points_cost;
  if (!finalPoints) {
    const cat = db.get('SELECT * FROM categories WHERE id = ?', category_id);
    if (cat) {
      const hours = (duration_minutes || 60) / 60;
      finalPoints = Math.round(hours * cat.base_rate * cat.multiplier);
    } else {
      finalPoints = 10;
    }
  }
  ...
  const result = db.run(
    'INSERT INTO services (provider_id, category_id, subcategory_id, title, description, points_cost, duration_minutes) VALUES (?, ?, ?, ?, ?, ?, ?)',
...
router.put('/:id', authMiddleware, (req: AuthRequest, res: Response) => {
  const service = db.get('SELECT * FROM services WHERE id = ? AND provider_id = ?', req.params.id, req.userId);
  ...
  db.run(`UPDATE services SET title = COALESCE(?, title), description = COALESCE(?, description),
    category_id = COALESCE(?, category_id), subcategory_id = COALESCE(?, subcategory_id),
    points_cost = COALESCE(?, points_cost), duration_minutes = COALESCE(?, duration_minutes),
...
router.delete('/:id', authMiddleware, (req: AuthRequest, res: Response) => {
  const result = db.run('DELETE FROM services WHERE id = ? AND provider_id = ?', req.params.id, req.userId);
