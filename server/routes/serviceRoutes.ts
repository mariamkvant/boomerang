...
router.get('/:id', async (req: AuthRequest, res: Response) => {
  ...
  const reviews = await db.all('SELECT r.id, r.request_id, r.reviewer_id, r.rating, r.comment, r.image, r.provider_reply, r.provider_reply_at, r.created_at, r.is_hidden, u.username as reviewer_name FROM reviews r JOIN users u ON r.reviewer_id = u.id JOIN service_requests sr ON r.request_id = sr.id WHERE sr.service_id = ? AND (r.is_hidden = false OR r.is_hidden IS NULL) ORDER BY r.created_at DESC', req.params.id);
...
router.put('/:id', authMiddleware, async (req: AuthRequest, res: Response) => {
  ...
  const { title, description, category_id, subcategory_id, points_cost, duration_minutes, is_active, is_product, image } = req.body;
  if (image !== undefined) {
    if (image && image.length > 5_000_000) return res.status(400).json({ error: 'Image too large (max 2MB)' });
    const imageUrl = image ? await uploadImage(image, 'boomerang/services') : null;
