...
export default function BottomNav() {
  ...
  const tab = (to: string, paths: string[], label: string, icon: JSX.Element, filledIcon?: JSX.Element) => {
    ...
    return (
      <Link to={to} className={`flex flex-col items-center justify-center flex-1 min-h-[44px] h-full gap-1 transition-colors ${active ? 'text-primary-500' : 'text-gray-400'}`}>
      ...
      <div className="flex items-center justify-around h-[60px] max-w-lg mx-auto">
        ...
        <Link to="/messages" className={`flex flex-col items-center justify-center flex-1 min-h-[44px] h-full gap-1 transition-colors ${isActive(['/messages']) ? 'text-primary-500' : 'text-gray-400'}`}>
          <div className="relative">
            ...
            {unreadDMs > 0 && (
              <span className="absolute -top-1 -right-2 min-w-[18px] h-[18px] bg-red-500 text-white text-[9px] font-bold rounded-full flex items-center justify-center px-1">{unreadDMs > 9 ? '9+' : unreadDMs}</span>
            )}
          </div>
          <span className="text-[11px] font-medium leading-none">{t('nav.chat')}</span>
        </Link>

        {/* Me tab — links to dashboard if pending actions, otherwise to /me */}
        <Link to={pendingActions > 0 ? '/dashboard' : '/me'}
          className={`flex flex-col items-center justify-center flex-1 min-h-[44px] h-full gap-1 transition-colors ${isActive(['/me', '/dashboard', '/settings', '/account', '/availability']) ? 'text-primary-500' : 'text-gray-400'}`}>
          <div className="relative">
            {isActive(['/me', '/dashboard', '/settings', '/account', '/availability']) ? (
            ...
            {pendingActions > 0 && <span className="absolute -top-1 -right-1 w-4 h-4 bg-primary-500 text-white text-[9px] font-bold rounded-full flex items-center justify-center">{pendingActions}</span>}
          </div>
