      ...
      <div className="bg-white dark:bg-[#202c33] rounded-2xl shadow-sm overflow-hidden mb-6">
        ...
        <div className="h-32 sm:h-40 relative overflow-hidden">
          {group.cover_image ? (
            ...
            <div className="w-full h-full bg-gray-100 dark:bg-[#242424]" />
          )}
          {isAdmin && (
        ...
        <div className="pt-12 px-6 pb-6">
          <div className="flex items-start justify-between">
            ...
            <div className="flex items-center gap-2 shrink-0 ml-4">
              {user && !isMember && (
                <button onClick={handleJoin} className="bg-gray-900 dark:bg-white text-white dark:text-gray-900 px-5 py-2 rounded-xl text-sm font-medium transition-colors">
                  {t('groups.join')}
                </button>
              )}
            ...
            <div className="mt-5 pt-5 border-t border-gray-100 dark:border-gray-700">
              <div className="flex flex-wrap gap-2">
                <button onClick={nativeShare}
                  className="flex items-center gap-2 bg-gray-100 dark:bg-[#242424] hover:bg-gray-200 dark:hover:bg-[#2a2a2a] px-4 py-2 rounded-xl text-sm font-medium transition-colors dark:text-white">
                ...
                <a href={`https://wa.me/?text=${encodeURIComponent(`Join "${group.name}" on Boomerang: ${shareUrl}`)}`}
                  ...
                  className="flex items-center gap-2 bg-gray-900 dark:bg-white text-white dark:text-gray-900 px-4 py-2 rounded-xl text-sm font-medium transition-colors">
      ...
      <div className="grid md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-2 space-y-6">
          ...
          {isAdmin && joinRequests.length > 0 && (
            <div className="bg-gray-50 dark:bg-[#1c1c1c] border border-gray-200 dark:border-gray-800 p-4 rounded-2xl">
              ...
              <div className="space-y-2">
                {joinRequests.map((jr: any) => (
                  <div key={jr.id} className="flex items-center justify-between bg-white dark:bg-[#242424] rounded-xl px-4 py-3">
                    ...
                    <div className="flex gap-2">
                      <button onClick={async () => { await api.approveJoinRequest(Number(id), jr.id); reload(); }}
                        className="text-xs bg-gray-900 dark:bg-white text-white dark:text-gray-900 px-3 py-1.5 rounded-lg font-medium">{t('groups.approve')}</button>
                      <button onClick={async () => { await api.denyJoinRequest(Number(id), jr.id); reload(); }}
                        className="text-xs bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 px-3 py-1.5 rounded-lg">{t('groups.deny')}</button>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="bg-white dark:bg-[#202c33] rounded-2xl shadow-sm p-4 mb-3">
                ...
                <div className="flex gap-2 mb-3">
                  <button onClick={() => setIsEvent(false)}
                    className={`flex-1 py-1.5 rounded-lg text-xs font-medium transition-colors ${!isEvent ? 'bg-gray-900 dark:bg-white text-white dark:text-gray-900' : 'bg-gray-100 dark:bg-[#242424] text-gray-500 dark:text-gray-400'}`}>
                    Post
                  </button>
                ...
                <div className="flex items-center justify-between mt-2 pt-2 border-t border-gray-100 dark:border-gray-700">
                  ...
                  <button onClick={async () => {
                    ...
                    className="bg-gray-900 dark:bg-white text-white dark:text-gray-900 px-4 py-1.5 rounded-lg text-xs font-medium hover:bg-gray-800 disabled:opacity-40 transition-colors">
                  ...
                  {announcements.map((a: any) => (
                    <div key={a.id} className={`bg-white dark:bg-[#202c33] rounded-2xl shadow-sm p-4 ${a.pinned ? 'border border-primary-200 dark:border-primary-800' : ''}`}>
                      ...
                      <div className="flex items-start gap-3 mt-1">
                        <div className="w-8 h-8 rounded-full bg-gray-200 dark:bg-gray-700 flex items-center justify-center text-gray-600 dark:text-gray-300 text-xs font-medium shrink-0">
