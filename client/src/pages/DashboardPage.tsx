      ...
      {user && (
        <div className="bg-white rounded-2xl shadow-card p-5 mb-6">
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              <h3 className="font-semibold text-sm text-gray-900 dark:text-white">Invite friends, earn boomerangs</h3>
              <p className="text-xs text-gray-500 mt-1">You both get 25 bonus boomerangs when they sign up.</p>
              <div className="flex flex-wrap gap-2 mt-3">
                <a href={`https://wa.me/?text=${encodeURIComponent(`Join me on Boomerang — exchange skills without money! ${window.location.origin}/register?ref=${user.id}`)}`}
                  ...
                  className="flex items-center gap-1.5 bg-[#25D366] text-white px-3 py-1.5 rounded-lg text-xs font-medium hover:bg-[#20bd5a] transition-colors">
                ...
                <button onClick={() => { navigator.clipboard.writeText(`${window.location.origin}/register?ref=${user.id}`); toast('Link copied!'); }}
                  className="flex items-center gap-1.5 bg-gray-100 dark:bg-[#2a3942] text-gray-600 dark:text-gray-300 px-3 py-1.5 rounded-lg text-xs font-medium hover:bg-gray-200 dark:hover:bg-[#3a4a54] transition-colors">
